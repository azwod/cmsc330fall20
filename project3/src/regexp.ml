...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  | Concat (x,y) -> let d = (List.hd (regexp_to_nfa x).fs, None, (regexp_to_nfa y).q0) in {sigma = (regexp_to_nfa x).sigma @ (regexp_to_nfa y).sigma; qs = (regexp_to_nfa x).qs @ (regexp_to_nfa y).qs; q0 = (regexp_to_nfa x).q0; fs = (regexp_to_nfa y).fs; delta = d::((regexp_to_nfa x).delta @ (regexp_to_nfa y).delta)}
