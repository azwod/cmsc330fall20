...
let new_states_helper (nfa: ('q, 's) nfa_t) (s: 's) (qs: 'q list) : 'q list = 
  let f a b = (match b with
                | (x,y,z) -> if List.mem x qs then (if y = (Some s) then(if List.mem z a then a else z::a) else a) else (if y = None && (List.mem x a)then(if List.mem z a then a else z::a)else a))
                                                in  List.fold_left f [] nfa.delta
