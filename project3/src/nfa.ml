...
let rec loop (nfa: ('q, 's) nfa_t) (s: 's) (l: 'q list) (qs: 'q list) : 'q list = 
     let f a b = (match b with
                | (x,y,z) -> if (y = (Some s)) && (List.mem x qs) then (if List.mem z a then a else (loop nfa s (z::a) qs)) else (if y = None && (List.mem x qs) then (if List.mem z a then a else (loop nfa s (z::a) qs) ) else a))
